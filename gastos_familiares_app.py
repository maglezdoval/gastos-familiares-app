        ...
        with tab_categorizar:
            ...
            if num_uncategorized > 0:
                ...
                if st.button("🤖 Sugerir CATEGORÍAS Faltantes", key="suggest_cats"):
                    ...
                    current_mask = (df_suggest[cat] == ph_cat); indices_to_update = df_suggest[current_mask].index
                    for index in indices_to_update:
                        row = df_suggest.loc[index]; sugg_cat, sugg_sub = suggest_category(row, desc, imp_calc, cat, subcat)
                        applied_c = False
                        ...
                        if sugg_sub and df_suggest.loc[index, subcat] == ph_sub:
                            if applied_c or (sugg_cat is None and df_suggest.loc[index, cat] != ph_cat): df_suggest.loc[index, subcat] = sugg_sub; applied_c = True
            ...
            display_mask = (df_display_edit[cat] == ph_cat) # Máscara basada solo en categoría
            ...
            if st.button("💾 Aplicar Cambios Editados", key="apply_manual_changes"):
                ...
                # Asegurar que los índices son únicos para evitar problemas con .loc
                if indices_edited.is_unique:
                    df_session.loc[indices_edited, edit_cols] = edited_data[edit_cols].values
                    # Calcular cambios comparando (un poco más complejo pero más seguro)
                    # O simplemente asumir que si se presiona el botón, algo cambió.
                    changes_manual = len(indices_edited) # Asumir cambios si se aplica
                    st.session_state.edited_df = df_session.copy()
                    st.success(f"{changes_manual} filas actualizadas en sesión (requiere re-ejecución para ver cambios en editor).")
                    convert_df_to_csv.clear(); st.experimental_rerun()
                else:
                    st.error("Error: Índices duplicados detectados al aplicar cambios. Contacta al desarrollador.")
                    # Fallback: Iterar como antes (menos eficiente pero más seguro con duplicados)
                    # ... (código de iteración anterior) ...
