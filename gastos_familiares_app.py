...
def learn_categories(df, concepto_col, cat_col, subcat_col, importe_col, placeholder_cat, placeholder_sub):
    ...
    # Aprender solo de filas donde AMBAS están definidas y no son las fijas TRASPASO/RECIBO
    df_categorized = df[
        (df[cat_col] != placeholder_cat) & (df[subcat_col] != placeholder_sub) &
        (~df[cat_col].isin(['TRASPASO', 'RECIBO'])) # Excluir las fijas del aprendizaje
    ].copy()
    ...
    st.sidebar.info(f"Aprendizaje: {len(category_knowledge['keyword_map'])} keywords (excl. TRASPASO/RECIBO).")

def suggest_category(row, concepto_col, importe_col, cat_col, subcat_col):
    # Esta función ya no necesita manejar TRASPASO/RECIBO porque se asignan antes
    ...
    # ... (resto de reglas explícitas como antes, EXCEPTO las de TRASPASO/RECIBO si las hubiera) ...
    ...
    # Devolver solo si se encontró algo, no devolver (None, None) por defecto
    return best_suggestion if best_suggestion else None # Devolver None si no hay sugerencia
...
def main():
    ...
    if st.session_state.edited_df is not None and not st.session_state.data_processed:
        ...
        try:
            ...
            # --- *** NUEVO: Aplicar categorías fijas basadas en TIPO *** ---
            mask_traspaso = df_processing[tipo] == 'TRASPASO'
            df_processing.loc[mask_traspaso, cat] = 'TRASPASO'
            # Opcional: Asignar subcategoría fija también
            df_processing.loc[mask_traspaso, subcat] = 'TRASPASO INTERNO' # O '' si prefieres

            mask_recibo = df_processing[tipo] == 'RECIBO'
            df_processing.loc[mask_recibo, cat] = 'RECIBO'
            # Opcional: Asignar subcategoría fija también
            df_processing.loc[mask_recibo, subcat] = 'PAGO RECIBO' # O '' si prefieres
            # --- Fin NUEVO ---
    ...
    if st.session_state.get('data_processed', False) and st.session_state.edited_df is not None:
        ...
        # Máscara y Conteo (basado solo en CATEGORÍA != placeholder Y != TRASPASO/RECIBO)
        uncategorized_mask = (df[cat] == ph_cat) # & (~df[cat].isin(['TRASPASO', 'RECIBO'])) # Ya no es necesario excluirlas aquí
        num_uncategorized = df[uncategorized_mask].shape[0]
        if num_uncategorized > 0: st.sidebar.warning(f"⚠️ {num_uncategorized} transacciones sin CATEGORÍA (excl. Trasp./Rec.).")
        ...
        with tab_categorizar:
            ...
            # Usar el num_uncategorized calculado al principio
            if num_uncategorized > 0:
                st.info(f"Hay {num_uncategorized} transacciones sin CATEGORÍA principal (excl. Trasp./Recibo).")
                # Botón Sugerir (ahora solo actúa sobre las SIN CATEGORÍA)
                if st.button("🤖 Sugerir CATEGORÍAS Faltantes", key="suggest_cats"):
                    ...
                    if imp_calc not in df_suggest.columns: raise KeyError("Falta importe en df_suggest")
                    ...
                    # --- Máscara y bucle ---
                    suggest_mask = (df_suggest[cat] == ph_cat) # Solo las SIN CATEGORÍA
                    indices_to_update = df_suggest[suggest_mask].index
                    for index in indices_to_update:
                        row = df_suggest.loc[index]
                        sugg_result = suggest_category(row, desc, imp_calc, cat, subcat)
                        applied_c = False
                        # Solo aplicar si hay sugerencia y la cat es placeholder
                        if sugg_result is not None: # suggest_category ahora devuelve None si no hay sugerencia
                           sugg_cat, sugg_sub = sugg_result # Desempaquetar solo si no es None
                           if sugg_cat and df_suggest.loc[index, cat] == ph_cat:
                               df_suggest.loc[index, cat] = sugg_cat
                               applied_c = True
                               # Aplicar subcategoría solo si también se sugirió y la actual es placeholder
            ...
            with col_f1:
                 # Checkbox usa la máscara recalculada al inicio de la pestaña
            ...
            display_mask = (df_display_edit[cat] == ph_cat) # Máscara para filtrar en editor
            ...
            cats_opts = sorted([str(c) for c in st.session_state.edited_df[cat].unique() if pd.notna(c) and c not in [ph_cat, 'TRASPASO', 'RECIBO']]) # Excluir fijas de opciones editables? Quizás no.
            subcats_opts = sorted([str(s) for s in st.session_state.edited_df[subcat].unique() if pd.notna(s) and s != ph_sub]) # Excluir fijas? Quizás no.
            ...
            if st.button("💾 Aplicar Cambios Editados", key="apply_manual_changes"):
                ...
                if indices_edited.is_unique:
                    ...
                    st.success(f"{changes_manual} filas actualizadas en sesión."); convert_df_to_csv.clear(); st.experimental_rerun()
                     ...
                     st.error("Error: Índices duplicados al aplicar cambios.") # Fallback omitido por brevedad
